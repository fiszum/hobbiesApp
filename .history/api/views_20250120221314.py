...
from .models import CustomUser, Hobby, FriendRequest, Thread
...
@login_required
"""
Update the hobbies of the authenticated user.

This view handles POST requests to update the hobbies of the authenticated user.
It expects a JSON request body containing a list of hobbies. Each hobby should
have a "name" field. The view will clear the user's current hobbies and add the
new ones provided in the request. It will also check and delete any hobbies that
are no longer associated with any user.
...
Returns:
    JsonResponse: A JSON response indicating the success or failure of the operation.
        - On success: {"message": "Hobbies updated successfully."}, status=200
        - On failure: {"error": "<error_message>"}, status=<error_status_code>
            - 400: If the JSON is invalid or the "hobbies" field is missing.
...
def thread_list(request):
    if request.method == "GET":
        threads = Thread.objects.order_by('-created_at').values('id', 'content', 'created_at', 'user__username')
    ...
    if request.method == "POST":
        ...
        if content:
            thread = Thread.objects.create(user=request.user, content=content)
            return JsonResponse({
                'id': thread.id,
